"""P2-0B.5: E2 Hook Capture tests (P2.x updated).

P2.x: Project Intent 和 Hook 端点已从生产服务器移除。
本测试验证这些端点不再存在，且生产代码不包含相关功能。
"""
...
class TestP2xHookArchitecture:
    def test_no_project_intent_endpoint(self, portal):
        """P2.x: /api/project-intent 端点不存在"""
        _, client = portal
        resp = client.post("/api/project-intent", json={"policy_id": "1", "need_description": "test"})
        assert resp.status_code == 404

    def test_no_project_hook_endpoint(self, portal):
        """P2.x: /api/project-hook 端点不存在"""
        _, client = portal
        resp = client.post("/api/project-hook", json={"project_intent_id": "test"})
        assert resp.status_code == 404
